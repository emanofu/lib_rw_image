...
# endif
	...
	const char *savepath = "/plugin/image/save/pic.";
	const char *ext =
...
# endif
	...
	igs::pic::writer pic_wr;
	igs::image::writer *p_wtr = &pic_wr;
#endif
...
if (b_pv_sw) {
	std::cout
		<< "open file tag is "
		<< std::boolalpha << p_rdr->istag(4,ca4)
		<< "\""
		<< (int)ca4[0]
		<< ","
		<< (int)ca4[1]
...
if (b_tv_sw) cl_stopw.start();
	...
	//p_rdr->segment(1000000);
		//	 4294967296
	//p_rdr->segment(2000000000);
	bool break_sw = false;
	p_rdr->get_data(
		argv[1]
		, prop
...
/******	igs::path::change_ext(
	...
	for (;ext_number<p_wtr->ext_count();++ext_number) {
		std::cout << "ext " << p_wtr->ext_lists()[ext_number] << '\n';
		if (p_wtr->ext_lists()[ext_number] == exten) {
			p_wtr->error_from_properties(
				prop,ext_number,error_or_warning_msg );
			if (0 < error_or_warning_msg.str().size()) {
				std::cout << error_or_warning_msg;
			}
			break;
		}
	}
